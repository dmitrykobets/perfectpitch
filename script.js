...
function parseArray(input) {
    ...
    while (i < input.length) {
        if (input[i] == "," || input[i] == "]") {
            ...
            i ++
        } else if (input[i] == "[" || input.slice(i).startsWith("\\[")) {
            let subArrOut = parseArray(input.slice(i))
            arr.push(subArrOut.arr)
            i += subArrOut.i + 1
        } else {
            let curTok = ""
    ...
    if (negated) {
        return {arr: new NegArray(arr), i}
    }
    return {arr, i}
}

class RandomLoop {
    constructor(multiplicity, notes, octaves) {
...
// i = starting point from which to go back (i.e., 12R --> i = 2, return 12)
function getLargestIntBackwards(input, i) {
    let num = NaN
    let readNum = 0
    i --
    do {
        readNum = parseInt(input.slice(i))
        if (!isNaN(readNum)) {
            num = readNum
            i --
        }
...
function parseTrainerInput(input) {
    stateStack = []
    ...
    while (i < input.length) {
        if (input[i] == 'R') {
            let multiplicity = getLargestIntBackwards(input, i) || 1

            i += 2
            let notes = []
            if (input[i] != ",") {
                let arr = parseArray(input.slice(i))
                notes = arr.arr
                i += arr.i + 1
            }
            ...
            randomLoops.push(new RandomLoop(multiplicity, notes, octaves))
        } else {
            i ++
        }
    }
    console.log(randomLoops)
    return randomLoops
}
